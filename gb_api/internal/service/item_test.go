...
// itemSvc builds an ItemSvc whose caller "testuser" has the given role and is
// assigned building 1 (with the given TypeAllowedSlot). A second user "other" exists
// so the "another person's board" authorization paths can be exercised.
func itemSvc(role uint, allowed map[uint][]uint) (*ItemSvc, *mock.ItemRepo) {
	r := &mock.ItemRepo{
	...
	users := &mock.AuthRepo{
		Roles:     map[string]uint{"testuser": role, "other": model.RoleStudent},
...
func newItemSvc(t *testing.T) (*ItemSvc, *mock.ItemRepo) {
	...
	return itemSvc(model.RoleTeacher, allowAll())
}

// newItemSvcAs builds an ItemSvc whose caller "testuser" has the given role.
func newItemSvcAs(role uint) (*ItemSvc, *mock.ItemRepo) {
	return itemSvc(role, allowAll())
}
...
func TestItemSvc_QueryItems_StudentOwnFull_OtherTypeOnly(t *testing.T) {
	...
	if one, ok := findItem(own.Inventory, 1); !ok || one.Type != 10 || one.QuestionID != 1 {
		t.Errorf("own board: expected full item 1, got %+v (found=%v)", one, ok)
	}
	...
	if len(other.Inventory) != 2 {
		t.Fatalf("other board: expected 2 items, got %d", len(other.Inventory))
	}
	for _, v := range other.Inventory {
		if v.ItemID != 0 || v.QuestionID != 0 {
			t.Errorf("other board: expected type-only view, got %+v", v)
		}
		if v.Type == 0 {
			t.Errorf("other board: expected a type, got %+v", v)
		}
	}
	if sv := other.Slots[0]; sv.ItemID != 0 || sv.QuestionID != 0 || sv.Type != 10 {
		t.Errorf("other board: expected type-only slot view, got %+v", sv)
	}
}
...
func TestItemSvc_TranInv2Slot_StudentOtherBoardForbidden(t *testing.T) {
...
func TestItemSvc_TranInv2Slot_TeacherOtherBoardForbidden(t *testing.T) {
	// The own-board rule is universal: even a teacher may only operate on their
