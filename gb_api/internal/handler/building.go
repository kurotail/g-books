...
// Update replaces the building identified by the {id} path segment (teacher/admin only).
func (h *BuildingHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	data, status, err := h.svc.Update(token, uint(id), req.Name, req.Layout, req.TypeAllowedSlot, req.DifficultyType)
	if err != nil {
		http.Error(w, err.Error(), status)
		return
	}
